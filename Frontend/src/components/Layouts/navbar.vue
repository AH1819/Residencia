<template>
  <div class="sticky top-2 z-40 mx-2 md:mx-4 mb-6">
    ...
    <!-- dropdown menu -->
    <div class="absolute bg-gray-100 border border-t-0 shadow-xl text-gray-700 rounded-b-lg w-48 top-full right-0 mr-6"
      ...
      <div class="max-w-xs" @click.stop>
        <div class="bg-white shadow-xl rounded-lg py-3">
          <div class="photo-wrapper p-2">
            <img class="w-32 h-32 rounded-full mx-auto" src="../../assets/profile.svg" alt="User Profile" />
          </div>
          <div class="p-10">
            <h3 class="text-center text-3xl text-gray-900 font-medium leading-8">
              {{ user.nombre_Doce + " " + user.apellido_paterno }}
            </h3>
            <div class="text-center text-gray-400 text-sm font-semibold" v-for="(item, index) in permisos" :key="index">
              <p>{{ item.Permiso }}</p>
            </div>
