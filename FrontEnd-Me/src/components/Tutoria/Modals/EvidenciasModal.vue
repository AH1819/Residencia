...
export default {
    ...
    return {
      ...
      columns: [
        ...
        { data: 'descripcionEvi' },
        {
          data: null,
          render: (data) => {
            const extension = data.urlEvi.split('.').pop().toLowerCase()
            let icono = 'file'
            if (extension === 'pdf') {
              icono = 'file-pdf'
        ...
        },
        {
          ...
          render: (data) => {
            ...
            <div class="w-full flex items-center justify-center">
              <button class="btn-editar-evidencia-tutoria bg-yellow-500 text-white p-2 pt-2 rounded mr-2" data-id="${data.idevidenciasT}"><i class="pi pi-pencil pointer-events-none"></i></button>
                        ...
                        <button class="btn-eliminar-evidencia-tutoria bg-red-500 text-white p-2 pt-2 rounded" data-id="${data.idevidenciasT}"><i class="pi pi-trash pointer-events-none"></i></button>
                    </div>
                      `
          }
        }
  ...
  mounted() {
    this.$nextTick(() => {
      document.addEventListener('click', (event) => {
        ...
        if (event.target.matches('.btn-eliminar-evidencia-tutoria')) {
          const id = event.target.getAttribute('data-id')
          this.eliminarEvidencia(id)
        }

        // Verificar si se hizo clic en el botón de editar
        if (event.target.matches('.btn-editar-evidencia-tutoria')) {
          const id = event.target.getAttribute('data-id')
  ...
  methods: {
    ...
    cargarEvidenciaParaEditar(id) {
      ...
      this.form.idEvidencia = evidencia.idevidenciasT
    ...
    submitForm() {
      if (this.archivo) {
        if (this.form.descripcionEvi === '') {
          this.form.descripcionEvi = 'Sin descripción'
        }

        const formData = new FormData()
        formData.append('idEvidenciasT', this.form.idEvidencia)
        formData.append('idActTutorias', this.form.id)
        ...
        if (!this.form.idEvidencia) {
          ...
          promise = apiTutorias.actualizarEvidencias(this.form.idEvidencia, formData)
        }
